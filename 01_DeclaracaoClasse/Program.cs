...
namespace _01_DeclaracaoClasse
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Conta pConta = new Conta(001, "1234-5", "67890-1", 1000.00m, 500.00m);
            Console.WriteLine($"Saldo inicial: {pConta.ConsultarSaldo():C}");
            pConta.Depositar(250.00m);

            Decimal valorSaque = 300.00m;
            pConta.Sacar(valorSaque);
            Console.WriteLine($"Saldo final: {pConta.ConsultarSaldo():C}");

            Decimal valorSaque2 = 2000.00m;
            pConta.Sacar(valorSaque2);

            Aluno alunoSouza = new Aluno();
            alunoSouza.código = 1;
            alunoSouza.nome = "João Souza";

            alunoSouza.LancarNota();
            Console.WriteLine($"Aluno {alunoSouza.nome} está {alunoSouza.Menção()}");
            alunoSouza.notas[0] = 4.0;

            alunoSouza.LancarNota();
            Console.WriteLine($"Aluno {alunoSouza.nome} está {alunoSouza.Menção()}");
            alunoSouza.notas[1] = 5.0;

            alunoSouza.LancarNota();
            Console.WriteLine($"Aluno {alunoSouza.nome} está {alunoSouza.Menção()}");
            alunoSouza.notas[2] = 6.0;

            alunoSouza.LancarNota();
            Console.WriteLine($"Aluno {alunoSouza.nome} está {alunoSouza.Menção()}");
            alunoSouza.notas[3] = 7.0;
            Console.ReadKey();
